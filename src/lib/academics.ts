...
export const listClassEnrollments=(class_id:string)=>supabase?supabase.from('student_enrollments').select('id,student_id,class_id,roll_number,active,enrolled_at').eq('class_id',class_id).eq('active',true).order('roll_number'):Promise.resolve({data:[],error:missing()});
