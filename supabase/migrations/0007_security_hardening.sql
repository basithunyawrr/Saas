...
-- Safe profile visibility: users see themselves; school admins can manage their school; teachers see names/roles through application queries without broad write access.
...
-- Membership/enrollment/links: school admins manage; teachers can read their school; parents only see their links.
...
-- Attendance: admins/teachers write; students/parents read only their applicable records.
...
create policy attendance_read on public.attendance for select using (
  public.is_active_super_admin() or public.can_manage_school(school_id) or
...
create policy attendance_teacher_write on public.attendance for insert with check (
  public.is_active_super_admin() or public.is_school_admin() and school_id=public.current_school_id() or
...
create policy attendance_teacher_update on public.attendance for update using (
  public.is_active_super_admin() or public.is_school_admin() or public.is_teacher()
) with check (school_id=public.current_school_id() or public.is_active_super_admin());
create policy attendance_admin_delete on public.attendance for delete using (public.is_active_super_admin() or public.is_school_admin());
...
-- Assignments: teachers/admins manage; students can read published assignments and manage their own submissions.
...
create policy submissions_student_read on public.assignment_submissions for select using (student_id=auth.uid() or public.is_active_super_admin() or public.can_manage_school(school_id) or public.is_teacher());
...
create policy submissions_student_update on public.assignment_submissions for update using (student_id=auth.uid() or public.is_active_super_admin() or public.is_school_admin() or public.is_teacher()) with check (school_id=public.current_school_id() or public.is_active_super_admin());
create policy submissions_staff_delete on public.assignment_submissions for delete using (public.is_active_super_admin() or public.is_school_admin());

-- Exams/results/report cards: staff manage; students/parents read applicable published data.
...
create policy exams_staff_write on public.exams for all using (public.is_active_super_admin() or public.is_school_admin() or public.is_teacher()) with check (school_id=public.current_school_id() or public.is_active_super_admin());
...
create policy results_read on public.exam_results for select using (
  public.is_active_super_admin() or public.can_manage_school(school_id) or student_id=auth.uid() or exists(select 1 from public.parent_student_links l where l.parent_id=auth.uid() and l.student_id=exam_results.student_id and l.school_id=exam_results.school_id)
);
create policy results_staff_write on public.exam_results for all using (public.is_active_super_admin() or public.is_school_admin() or public.is_teacher()) with check (school_id=public.current_school_id() or public.is_active_super_admin());
...
create policy reports_read on public.report_cards for select using (
  public.is_active_super_admin() or public.can_manage_school(school_id) or (student_id=auth.uid() and published=true) or exists(select 1 from public.parent_student_links l where l.parent_id=auth.uid() and l.student_id=report_cards.student_id and l.school_id=report_cards.school_id and report_cards.published=true)
);
create policy reports_staff_write on public.report_cards for all using (public.is_active_super_admin() or public.is_school_admin() or public.is_teacher()) with check (school_id=public.current_school_id() or public.is_active_super_admin());

-- Announcements: school staff create; everyone in school reads published items.
...
create policy announcements_staff_write on public.announcements for all using (public.is_active_super_admin() or public.is_school_admin() or public.is_teacher()) with check (school_id=public.current_school_id() or public.is_active_super_admin());
...
-- Central plan limit enforcement for students, teachers and classes.
